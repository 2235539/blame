...
PROBLEM:
   ...
   Apply the Selection Sort algorithm (Filename: FinalExercise4C.java)
...
public class FinalExercise4C {
   public static void main(String[] args) {
       ...
       {
           int index = i;
           for (int j = i + 1; j < numberOfStudents.length; j++){
               if (numberOfStudents[j] < numberOfStudents[index]){
                   index = j;
               }
           }
           int smallerNumber = numberOfStudents[index];
           numberOfStudents[index] = numberOfStudents[i];
           numberOfStudents[i] = smallerNumber;
       }
