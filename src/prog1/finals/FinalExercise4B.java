...
PROBLEM:
   ...
   Apply the Bubble Sort algorithm (Filename: FinalExercise4B.java)
...
public class FinalExercise4B {
   public static void main(String[] args) {
       ...
       int temp = 0;
       for (int i = 0; i < numberOfStudents.length; i++) {
           for (int j = 1; j < (numberOfStudents.length - i); j++) {
               if (numberOfStudents[j - 1] > numberOfStudents[j]) {
                   temp = numberOfStudents[j - 1];
                   numberOfStudents[j - 1] = numberOfStudents[j];
                   numberOfStudents[j] = temp;
               }
           }
       }
